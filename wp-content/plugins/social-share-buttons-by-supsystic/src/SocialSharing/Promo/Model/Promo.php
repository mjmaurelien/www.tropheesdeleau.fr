...
class SocialSharing_Promo_Model_Promo extends SocialSharing_Core_BaseModel
{
    ...
    public  function createProjectDesignTour()
    {
        ...
        $content  = '<h3>' . $this->environment->translate('Choose button design') . '</h3>'
            .'<p>'
                . $this->environment->translate('Choose one of the button design. You can change design later and buttons settings on the next step.')
            .'</p>';
        $opt_arr  = array(
            'content'  => $content,
        ...
        $this->_buttonArray['startNextStep']['function'] = '
            ...
            $("button.select-all").pointer(social_share_buttons_pointer_options).pointer("open");
    ...
    public function createProjectNetworksTour()
    {
        ...
        $content  = '<h3>' . $this->environment->translate('Choose social networks') . '</h3>'
            .'<p>'
                . $this->environment->translate('You can change social networks later.')
            .'</p>';
        $opt_arr  = array(
            'content'  => $content,
    ...
    public function printScripts( $selector, $options, $windowVisibility = 'open')
    {
        ...
        <script type="text/javascript">
            ...
            (function ($) {
                    ...
                    var social_share_buttons_pointer_options = <?php echo $this->jsonEncode( $options ); ?>, setup;

                    social_share_buttons_pointer_options = $.extend(social_share_buttons_pointer_options, {
                    ...
                    setup = function () {
                        $('<?php echo $selector; ?>').pointer(social_share_buttons_pointer_options).pointer("<?php echo $windowVisibility?>");
                    ...
                    if (social_share_buttons_pointer_options.position && social_share_buttons_pointer_options.position.defer_loading)
